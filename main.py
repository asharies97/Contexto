...
from custom_types import RAGQueryResults, RAGSearchResult, RAGUpsertResult, RAGChunksAndSrc
...
async def rag_query_pdf_ai(ctx: inngest.Context):
    async def _search(question:str, top_k=5):
        print()
        query_vec=embed_texts([question])[0]
        store=QdrantStorage()
        found=await store.search(query_vec,top_k=top_k)
    ...
    res=await ctx.step.ai.infer(
        ...
        body={
            ...
            "temperature":0.2,
            "messages":[
                {"role":"system", "content":"Answer question using only provided context."},
